...
void aesblock_rotatexor(unsigned long *block)
{
	unsigned long carry=0;

	carry = block[3]>>31;
	block[3] = (block[2]>>31) | (block[3]<<1);
	block[2] = (block[1]>>31) | (block[2]<<1);
	block[1] = (block[0]>>31) | (block[1]<<1);
	block[0] = block[0]<<1;
	
	if(carry)block[0] ^= 0x87;
}

void dsi_encrypt_ccm_macgencmd9( dsi_context* ctx, 
					  unsigned char* input, 
					  unsigned char* output,
					  unsigned int size,
					  unsigned char* mac )
{
	...
	memset(stream, 0, 16);
	aes_crypt_ecb(&ctx->aes, AES_ENCRYPT, block, stream);//AES-CTR /w CTR=0
	for(i=0; i<16; i++)
			cryptblock[i] = stream[15-i];

	memcpy(&block[16-size], input, size);
	if(size<16)block[16-size-1] = 0x80;
	aesblock_rotatexor((unsigned long*)cryptblock);
	if(size<16)aesblock_rotatexor((unsigned long*)cryptblock);

	for(i=0; i<4; i++)((unsigned long*)block)[i] ^= ((unsigned long*)cryptblock)[i];
